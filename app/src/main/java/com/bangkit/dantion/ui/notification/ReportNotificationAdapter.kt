...
import com.bangkit.dantion.utils.DetectionReportDiffCallback

class ReportNotificationAdapter(private val reportList: ArrayList<DetectionReportEntity>, private val onClick : (DetectionReportEntity) -> Unit) :
    RecyclerView.Adapter<ReportNotificationAdapter.ViewHolder>() {

    fun updateData(items: ArrayList<DetectionReportEntity>) {
        val diffCallback = DetectionReportDiffCallback(reportList, items)
    ...
    inner class ViewHolder(private val binding: NotificationItemBinding) :
        ...
        fun bind(data: DetectionReportEntity) {
            val context = itemView.context
            if (data.isRead)
                binding.cardNotif.alpha = .5f
    ...
    private fun setStatus(binding: NotificationItemBinding, data: DetectionReportEntity, context: Context){
